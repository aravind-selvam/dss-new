...
from common.util import nearest_valid_size
...
class PLSleepModel(LightningModule):
    def __init__(self, cfg, val_event_df, feature_dim, num_classes, duration):
        super().__init__()
        self.cfg = cfg
        self.val_event_df = val_event_df
        num_timesteps = nearest_valid_size(int(duration * self.cfg["upsample_rate"]), self.cfg["downsample_rate"])
        self.model = get_model(
            cfg,
            feature_dim=feature_dim,
            n_classes=num_classes,
            num_timesteps=num_timesteps // self.cfg["downsample_rate"],
        )
        self.duration = duration
        self.validation_step_outputs: list = []
        self.__best_loss = np.inf

    def forward(self, x, labels, do_mixup, do_cutmix):
