...
from common.util import gaussian_label, nearest_valid_size, negative_sampling, pad_if_needed, random_crop


###################
# Label
###################
def get_centernet_label(this_event_df, num_frames, duration, start, end):
...
class CenterNetTrainDataset(Dataset):
    def __init__(self, cfg, features, event_df):
        self.cfg = cfg
...
class CenterNetValidDataset(Dataset):
    def __init__(self, cfg, chunk_features, event_df):
        self.cfg = cfg
...
class CenterNetTestDataset(Dataset):
    def __init__(self, cfg, chunk_features):
        self.cfg = cfg
        self.chunk_features = chunk_features
        self.keys = list(chunk_features.keys())
        self.num_features = len(cfg["features"])
